...
public:
    ...
    template <typename Vec>
    Vec transform_point(const Vec& _vec) const
    {
        auto res = Vec(0,0,0);
    ...
    template <typename Vec>
    Vec transform_vector(const Vec& _vec) const
    {
        auto res = Vec(0,0,0);
        for (unsigned int i = 0; i < 3; ++i)
            for (unsigned int j = 0; j < 3; ++j)
    ...
    template <typename Vec>
    std::vector<Vec> transform_vectors(const std::vector<Vec>& vecs) const
    {
        auto res = std::vector<Vec>();
        res.reserve(vecs.size());
        for (const auto& vec : vecs) res.push_back(transform_vector(vec));
        return res;
    }
...
using Matrix4x4d = HexHex::Matrix4x4T<double>;
